...
app = FastAPI(title="GrantFlow AI API", version="1.0.0")
...
def verify_eligibility(request: VerifyRequest):
    ...
    history_mgr.save_full_analysis(
        bando_titolo=scheda[:80] if scheda else "Bando",
        azienda_nome=d.ragione_sociale,
        ateco=d.ateco,
        fatturato=d.fatturato,
        investimento=d.investimento,
        finanziamento=d.finanziamento_richiesto,
        ...
        scheda=scheda,
        esito=eligibility,
        progetto=business_plan,
        parametri_finanziari=json.dumps(pf, ensure_ascii=False),
        calcolo_finanziario=json.dumps(calcolo, ensure_ascii=False),
    )
...
@app.get("/history")
def get_history():
    return history_mgr.get_all()


@app.delete("/history/{analysis_id}")
def delete_history(analysis_id: str):
    history_mgr.delete_by_id(analysis_id)
